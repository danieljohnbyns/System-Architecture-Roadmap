 ...
 * **Core Concepts**: Scalability (horizontal/vertical), reliability (MTTF, MTTR, SLAs/SLOs/SLIs), observability (logs, metrics, traces), performance, and security.
 * **Practical Output**: Define SLOs for a service. Design a scalable architecture for a known problem (e.g., "Design a URL shortener") that addresses specific NFRs.
 ...
 * **Deliverable**: A one-page project charter.
     * ne the **core business problem**.
     * st key **functional requirements** (what the system does) and **non-functional requirements** (scale, reliability, security).
     * fy the primary **stakeholders** (e.g., end-users, operators, business owners).
 ...
 * **Deliverables**:
     ...
     * **Architecture Decision Record (ADR) Log**: At least 5 ADRs documenting critical technology choices (e.g., database selection, communication protocol, cloud provider) with justification.
     ...
     * **Non-Functional Requirement (NFR) Plan**: Explicit SLOs/SLIs and a description of how scalability, reliability, and security will be achieved.
